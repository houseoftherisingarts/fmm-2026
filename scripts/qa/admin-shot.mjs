...
// Vite doit tourner : npx vite --port 5199 --strictPort. Ferme les pop-ups sans rien réclamer, passe la porte CA, ouvre la section.
//   JETON=/tmp/jeton-fmm.txt node scripts/qa/admin-shot.mjs /admin/benevoles scripts/qa/shots/admin-benevoles
...
for (const [w, h, tag] of (process.env.ONLY ? [[390, 844, '390']] : [[1440, 900, '1440'], [390, 844, '390']])) {
  ...
  const entrer = p.getByText(/^\s*entrer\s*$/i).first();
  if (await entrer.count()) { await entrer.scrollIntoViewIfNeeded().catch(()=>{}); await entrer.click({ force: true }).catch(()=>{}); await p.waitForTimeout(5000); }
  if (!/Liste d.attente 2027/.test(await p.evaluate(() => document.body.innerText))) {
  ...
  console.log(tag, 'complete-banner:', /Équipe 2026 complète/.test(txt), '| bouton:', /Rouvrir le recrutement|Déclarer l.équipe complète/.exec(txt)?.[0], '| filtre:', /Liste d.attente 2027/.test(txt));
  await c.close();
}
await b.close();
