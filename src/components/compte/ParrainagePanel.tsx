  ...
  return (
    <section className="glass-light rounded-lg-card p-7 md:p-8">
      <div className="flex items-center justify-between gap-4 mb-5 pb-2" style={{ borderBottom: '1px solid rgba(244, 239, 227, 0.10)' }}>
        ...
        <span className="font-sans text-sm tracking-[0.2em]" style={{ color: '#D8B05A', fontWeight: 300 }}>{n}</span>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        <span className="px-5 py-3 rounded-card font-display text-2xl tracking-[0.3em]"
              style={{ background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(232,177,74,0.35)', color: '#F4EFE3' }}>
          {code || '······'}
        </span>
        ...
        {PALIERS_PARRAINAGE.map((p) => {
          ...
          return (
            <li key={p.filleuls} className="flex items-center gap-3 px-4 py-2.5 rounded-card"
                style={{ background: atteint ? 'rgba(216,176,90,0.10)' : 'rgba(26,5,11,0.4)', border: `1px solid ${atteint ? 'rgba(216,176,90,0.45)' : 'rgba(244,239,227,0.10)'}` }}>
              <Icone size={13} style={{ color: atteint ? '#D8B05A' : 'rgba(244,239,227,0.4)' }} />
              <span className="font-sans text-xs uppercase tracking-[0.16em] w-32 shrink-0"
                    style={{ color: atteint ? '#F4EFE3' : 'rgba(244,239,227,0.5)' }}>
                {fr ? p.cleFR : p.cleEN}
              </span>
              <span className="font-editorial text-sm flex-1" style={{ color: atteint ? 'var(--color-amber-glow)' : 'rgba(244,239,227,0.55)' }}>
                {fr ? p.recompenseFR : p.recompenseEN}
              </span>
              {atteint && <Check size={13} style={{ color: '#D8B05A' }} />}
            </li>
          );
        })}
      </ul>
      {prochain && (
        <p className="font-sans text-xs mb-6" style={{ color: 'rgba(244,239,227,0.55)' }}>
          {fr
      ...
      {parrain && (
        <p className="font-sans text-xs mb-3" style={{ color: 'rgba(244,239,227,0.55)' }}>
          {fr ? 'Vous êtes entré grâce au code ' : 'You joined with the code '}<span style={{ color: '#D8B05A' }}>{parrain.code}</span>.
        </p>
      )}
      {filleuls.length === 0 ? (
          ...
          {filleuls.map((f) => (
            <li key={f.filleulUid}>
              <Link to={`${addLocale('/profil', lang)}/${f.filleulUid}`}
                ...
                <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: '#D8B05A' }} />
                {f.filleulNom || (fr ? 'Un inconnu' : 'A stranger')}
              </Link>
              {(petitsFilleuls[f.filleulUid] || []).length > 0 && (
                <ul className="ml-5 mt-1 space-y-1" style={{ borderLeft: '1px solid rgba(244,239,227,0.14)' }}>
