...
import { suivreFiches } from '../firebase/activites';
...
// Les fiches d'activités viennent de Firestore (modifiables dans
// l'admin, section Activités). Tant que la collection est vide, celles
// écrites dans le code font foi : la page n'est jamais vide.
interface Activite {
...
const depuisFiche = (f: { titreFR: string; titreEN: string; sousTitreFR: string; sousTitreEN: string;
  descFR: string; descEN: string; image: string; imagePos?: string; categorie: string; retiree?: boolean }): Activite => ({
  titleFR: f.titreFR, titleEN: f.titreEN,
  bodyFR: f.sousTitreFR, bodyEN: f.sousTitreEN,
  descFR: f.descFR, descEN: f.descEN,
  image: f.image, imagePos: f.imagePos,
  category: f.categorie as Category,
  retiree: f.retiree,
});

const ACTIVITES_FILET: Activite[] = ACTIVITES_DU_CODE.map(depuisFiche);
...
const ActivitesPage: React.FC<{ embedded?: boolean }> = ({ embedded = false }) => {
  ...
  // Les fiches d'activités, en direct depuis l'admin.
  const [fichesLive, setFichesLive] = useState<Activite[] | null>(null);
  useEffect(() => suivreFiches((f) => setFichesLive(f.length ? f.map(depuisFiche) : null)), []);
  const ACTIVITIES = fichesLive ?? ACTIVITES_FILET;
